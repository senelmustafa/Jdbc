...
==============================================================================*/
    ...
    -- örnek 1 ismi i null olanları sorgulayınız
    select * from insanlar
    where isim is null;
        
          -- örnek 1 ismi i null olmayanları sorgulayınız
    select * from insanlar
...
-- tabloyu eski haline  döndürünüz
update insanlar
set isim=null
where isim='no name';
...
set isim=coalesce(isim,'henüz isim girilmedi'),
    ...
    ssn=coalesce(ssn,'no SSN');-- coalesce null ları değiiştir
...
/* ----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
 select* from kisiler
 order by adres; -- adrese göre sırala küçükten büyüğe
 /* ----------------------------------------------------------------------------
  ORNEK1: kisiler tablosunu maasa  göre ters sıralayarak sorgulayınız.
 -----------------------------------------------------------------------------*/ 
 select* from kisiler
 order by maas desc; -- adrese göre sırala küçükten büyüğe
 /* ----------------------------------------------------------------------------
  ORNEK4: ismi Mine olanları, SSN'e göre AZALAN sırada sorgulayınız.
-----------------------------------------------------------------------------*/
    select * from kisiler
    where isim='mine'
    order by ssn desc;  -- desc tersten sıralar
/* ----------------------------------------------------------------------------
  ORNEK5: soyismi 'i Bulut olanları maas sıralı olarak sorgulayınız.
-----------------------------------------------------------------------------*/ 
select * from kisiler
where soyisim='bulut'
order by 5;   -- 5 sutun maas denk geliyor
...
select * from kisiler limit 10,2;   -- ilk sayı dahil değil ikincisi dahil
-- select * from kisiler where id>5 limit 2; -- limitli soru değil
...
-----------------------------------------------------------------------------*/
select * from kisiler 
order by maas desc -- sondan başa doğru sırala
limit 3;
/* ORACLE çözümü
select * from kisiler 
order by maas desc
